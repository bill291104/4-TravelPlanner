...
def create_embedding(request: EmbeddingRequest, db_collection):
    """
...
def create_embeddings_batch(requests: List[EmbeddingRequest], db_collection):
    """
...
def delete_embedding(pk: int, db_collection):
    """
    주어진 pk(id)에 해당하는 문서를 DB에서 삭제합니다.
    """
    db_collection.delete(ids=[str(pk)])

def delete_embeddings_batch(pks: List[int], db_collection):
    """
