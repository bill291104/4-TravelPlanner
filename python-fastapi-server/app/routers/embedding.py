...
async def embedding_travel_data(
    ...
    try:
        # 두 번째 블록의 try/except를 유지하면서, 첫 번째 블록처럼 'await'를 추가
        await embedding_service.create_embedding(request, db_collection) # <-- 'await' 추가!
        logger.info(f"pk='{request.pk}' 문서를 '{domain.value}' 컬렉션에 성공적으로 임베딩했습니다.") # logger.info 유지
    except Exception as e:
        logger.error(f"단일 임베딩 실패: domain='{domain.value}', pk='{request.pk}', error='{e}'") # logger.error 유지
